...
export default function ToolCard({title, desc, to, children, disabled = false}) {
  return (
    <Link to={to} className={`flex flex-col gap-2 min-h-40 rounded-xl border border-white/10 bg-[#181818] p-4 ${disabled ? "opacity-50 cursor-not-allowed" : "opacity-100"}`}>
...
function ToolCardIcon({svg}){
    return(
        <span className={`text-special`}>{svg}</span>
    )
}
...
function ToolCardDescription({description}){
    return(
        <p className="mt-3 text-md text-neutral-300">{description}</p>
    )
}
